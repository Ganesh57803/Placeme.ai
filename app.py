...
class ResumeApp:
    def __init__(self):
        ...
        self.gemini_api = GeminiAPI(api_key="yourgeminiapikey")
        ...
        self.pages = {
            ...
            "🔍 RESUME ANALYZER": self.render_analyzer2,
    ...
    def render_analyzer2(self):
        st.markdown("<h1 style='text-align: center;'>📄 Resume Analyzer</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center;'>Upload your resume and get <b>AI-powered</b> insights instantly!</p>", unsafe_allow_html=True)

        uploaded_file = st.file_uploader("📤 Upload your resume (PDF or DOCX)", type=['pdf', 'docx'])

        if uploaded_file:
            try:
                raw_text = self.parser.extract_text(uploaded_file)
                st.success("✅ Resume text extracted successfully!")

                with st.spinner("🔍 Analyzing your resume..."):
                    structured_data = self.gemini_api.analyze_resume(raw_text)

                self.display_resume_data(structured_data)
    ...
    def display_resume_data(self, data):
        """Display structured resume data in a beautiful UI"""
        st.header("Resume Details")

        # Parse the JSON content from the Gemini API response
        resume_data = self.parse_resume_data(data)
        if not resume_data:
            return

        self.display_personal_info(resume_data)
        self.display_skills(resume_data)
        self.display_work_experience(resume_data)
        self.display_education(resume_data)
        self.display_projects(resume_data)
        self.display_extracurricular_activities(resume_data)

    def parse_resume_data(self, data):
        """Parse the JSON content from the Gemini API response"""
        try:
            ...
            cleaned_json = raw_json.strip("```json").strip("```")
            print(f"Cleaned JSON: {cleaned_json}")
            # Parse the cleaned JSON string
            return json.loads(cleaned_json)
        except Exception as e:
            st.error(f"Failed to parse the resume data: {str(e)}")
            return None
    def display_personal_info(self, resume_data):
        st.subheader("👤 Personal Information")

        col1, col2 = st.columns(2)

        name = resume_data.get('Name', resume_data.get('name', 'N/A'))
        contact = resume_data.get("Contact Information", resume_data.get("contact_information", {}))

        with col1:
            st.markdown(f"**Name:** {name}")
            st.markdown(f"**Phone:** {contact.get('Phone', contact.get('phone', 'N/A'))}")
            st.markdown(f"**Email:** {contact.get('Email', contact.get('email', 'N/A'))}")

        with col2:
            st.markdown(f"**Website:** {contact.get('Website', contact.get('website', 'N/A'))}")
            st.markdown(f"**LinkedIn:** {contact.get('LinkedIn', contact.get('linkedin', 'N/A'))}")
            st.markdown(f"**GitHub:** {contact.get('Github', contact.get('github', 'N/A'))}")
    def display_resume_data(self, data):
        ...
        tabs = st.tabs(["💼 Experience", "🛠 Skills", "🎓 Education", "📂 Projects", "🎨 Extras"])
    ...
    def display_skills(self, resume_data):
        st.subheader("🛠 Skills")
    ...
    # Split skills into rows of 4
        ...
        for category, skills in resume_data.get("Skills", resume_data.get("skills", [])).items():
    ...
    def display_work_experience(self, resume_data):
        st.subheader("💼 Work Experience")
        experiences = resume_data.get("Work Experience", resume_data.get("work_experience", []))

        if experiences:
            for i, exp in enumerate(experiences):
                with st.expander(f"📁 {exp.get('Role', exp.get('role', 'Unknown Role'))} at {exp.get('Company', exp.get('company', 'Unknown Company'))}"):
                    st.markdown(f"**Duration:** {exp.get('Duration', exp.get('duration', 'N/A'))}")
                    st.markdown(f"**Description:**\n{exp.get('Description', exp.get('description', 'N/A'))}")
        else:
            st.markdown("No work experience found.")


    def display_education(self, resume_data):
        ...
        if education:
            for edu in education:
                with st.expander(f"🎓 {edu.get('Degree', 'Degree')} @ {edu.get('Institution', 'Institution')}"):
                    st.markdown(f"**Year:** {edu.get('Year', edu.get('year', 'N/A'))}")
                    st.markdown(f"**CGPA/Percentage:** {edu.get('CGPA', edu.get('cgpa', edu.get('percentage', 'N/A')))}")
        else:
            st.markdown("No education details found.")

    def display_projects(self, resume_data):
        st.subheader("📂 Projects")
        projects = resume_data.get("Projects", resume_data.get("projects", []))

        if projects:
            for project in projects:
                with st.expander(f"🧪 {project.get('Name', 'name')}"):
                    technologies = project.get('Technologies', project.get('technologies', []))
                    tech_list = ", ".join(technologies) if isinstance(technologies, list) else technologies
                    st.markdown(f"**Technologies:** {tech_list}")
                    st.markdown(f"**Description:** {project.get('Description', project.get('description', 'N/A'))}")
        else:
    ...
    def display_extracurricular_activities(self, resume_data):
        st.subheader("🎨 Extracurricular Activities")
        extras = resume_data.get("Extracurricular Activities", resume_data.get("extracurricular_activities", []))

        if extras:
            st.markdown("\n".join([f"- {activity}" for activity in extras]))
        else:
            st.markdown("No extracurricular activities found.")
